...
public class CommandHealth extends CommandBase
{
	...
	{
		if (args.length == 0) throw new WrongUsageException("commands.health.usage", new Object[0]);
		else
        {
            String arg = args[0];
            int health = parseInt(arg);
            
            EntityPlayer playerIn = args.length > 1 ? getPlayer(server, sender, args[1]) : getCommandSenderAsPlayer(sender);
            notifyCommandListener(sender, this, "commands.health.success", new Object[] {playerIn.getName(), health});
        	playerIn.setHealth(health);
