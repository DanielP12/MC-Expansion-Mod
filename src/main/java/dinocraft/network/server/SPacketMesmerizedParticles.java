...
public class SPacketMesmerizedParticles extends ParticlePositionPacket<SPacketMesmerizedParticles>
{
	private int particleCount1, particleCount2;
	
	public SPacketMesmerizedParticles()
	{

	}
	
	public SPacketMesmerizedParticles(int count1, int count2, float xCoord, float yCoord, float zCoord, float xOffset, float yOffset, float zOffset)
	{
	...
	@Override
	public void handleClientSide(SPacketMesmerizedParticles message, EntityPlayer player)
	{
		for (int i = 0; i < message.particleCount1; ++i)
		{
			player.world.spawnParticle(EnumParticleTypes.SPELL_WITCH, true,
		...
		{
			player.world.spawnParticle(EnumParticleTypes.END_ROD, true,
					message.xCoord - message.xOffset + 2.0F * message.xOffset * player.world.rand.nextFloat(),
					message.yCoord - message.yOffset + 2.0F * message.yOffset * player.world.rand.nextFloat(),
					message.zCoord - message.zOffset + 2.0F * message.zOffset * player.world.rand.nextFloat(),
	...
	@Override
	public void handleServerSide(SPacketMesmerizedParticles message, EntityPlayer player)
	{
