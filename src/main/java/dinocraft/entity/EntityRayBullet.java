...
public class EntityRayBullet extends EntityThrowable 
{
	...
	public EntityRayBullet(World worldIn) 
	{
	    super(worldIn);
	}
	
	/** The entity shooting this bullet */
	public Entity shootingEntity;
	
	public EntityRayBullet(World worldIn, EntityLivingBase shooter)
	{
	    super(worldIn, shooter);
        this.shootingEntity = shooter;
	}

	public EntityRayBullet(EntityLivingBase shooter, float gravityIn)
	{
	...
	public void handleStatusUpdate(byte id) 
	{
		if (id == 3)
		{
			...
			worldIn.playSound(this.posX, this.posY, this.posZ, DinocraftSoundEvents.CRACK, SoundCategory.NEUTRAL, 1.0F, rand.nextFloat() + 1.0F, false);
			
			for (int i = 0; i < 16; ++i)
			{
				worldIn.spawnParticle(EnumParticleTypes.ITEM_CRACK, this.posX, this.posY, this.posZ, Math.random() * 0.2 - 0.1, Math.random() * 0.25, Math.random() * 0.2 - 0.1, Item.getIdFromItem(DinocraftItems.RAY_BULLET));
			}
		}
	}
/*
	...
	protected void onImpact(RayTraceResult result)
	{
        ...
        if (result.entityHit != null && result.entityHit instanceof EntityLivingBase)
        {
            ...
            //{
   				if (result.entityHit != this.shootingEntity)
   			 	{
            		worldIn.playSound(null, entity.getPosition(), DinocraftSoundEvents.HIT, SoundCategory.NEUTRAL, 1.0F, rand.nextFloat() + 1.0F);            	
   			 		entity.attackEntityFrom(DamageSource.causeThrownDamage(this, getThrower()), 4.0F);
   			 	}
   			 	else if (this.shootingEntity != null && this.shootingEntity instanceof EntityPlayer && result.entityHit == this.shootingEntity)
   			 	{
   			 		this.setDead();
	...
	public void onUpdate() 
	{
		...
		this.world.spawnParticle(EnumParticleTypes.CLOUD, true, this.posX, this.posY, this.posZ, 0, 0, 0, 1);
		super.onUpdate();
	}
