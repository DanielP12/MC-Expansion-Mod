...
public class EntitySpikeBall extends EntityThrowable
{
	private static final DataParameter<Float> GRAVITY = EntityDataManager.createKey(EntitySpikeBall.class, DataSerializers.FLOAT);

	public EntitySpikeBall(World world)
	{
		super(world);
	}

	public EntitySpikeBall(World world, EntityLivingBase thrower)
	{
		super(world, thrower);
	}
	
	public EntitySpikeBall(EntityLivingBase thrower, float gravity)
	{
	...
	public void handleStatusUpdate(byte id)
	{
		if (id == 3)
		{
			...
			for (int i = 0; i < 16; ++i)
			{
				this.world.spawnParticle(EnumParticleTypes.ITEM_CRACK, this.posX, this.posY, this.posZ, Math.random() * 0.2 - 0.1, Math.random() * 0.25, Math.random() * 0.2 - 0.1, Item.getIdFromItem(DinocraftItems.SPIKE_BALL));
			}
		}
	}
	...
	protected void onImpact(RayTraceResult result)
	{
		...
		if (!this.world.isRemote && result != null)
		{
			...
			{
				result.entityHit.attackEntityFrom(this.thrower != null ? DamageSource.causeThrownDamage(this, this.thrower) : DamageSource.GENERIC, 9.5F);
				((EntityLivingBase) result.entityHit).addPotionEffect(new PotionEffect(MobEffects.POISON, 100, 1));
